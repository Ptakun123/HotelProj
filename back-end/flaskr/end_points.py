...
def getfree():
    try:
        ...
        if not city or not start_date or not end_date or not guests:
            return jsonify({"error": "Brak wymaganych parametrów: city, start_date, end_date, guests"}), 400
        ...
        try:
            ...
            return jsonify({"error": "Nieprawidłowy format daty lub liczby gości. Użyj formatu YYYY-MM-DD dla dat i liczby całkowitej dla gości"}), 400

        if start_date >= end_date:
            return jsonify({"error": "Data początkowa musi być wcześniejsza niż data końcowa"}), 400
        ...
        if not rooms:
            return jsonify({"message": "Brak dostępnych pokoi w podanym mieście, terminie i dla podanej liczby gości"}), 404

        return jsonify({"available_rooms": rooms}), 200

    except Exception as e:
        return jsonify({"error": "Wystąpił błąd podczas przetwarzania zapytania", "details": str(e)}), 500
...
def reserve():
    try:
        ...
        if missing_fields:
            return jsonify({"error": "Brak wymaganych pól", "missing_fields": missing_fields}), 400

        try:
            first_night = datetime.strptime(data['first_night'], '%Y-%m-%d').date()
            last_night = datetime.strptime(data['last_night'], '%Y-%m-%d').date()
        except ValueError:
            return jsonify({"error": "Nieprawidłowy format daty. Użyj formatu YYYY-MM-DD"}), 400
        ...
        if query:
            return jsonify({"error": "Pokój jest już zarezerwowany w podanym terminie"}), 400
        ...
        db.session.rollback()
        return jsonify({"error": "Wystąpił błąd podczas tworzenia rezerwacji", "details": str(e)}), 500